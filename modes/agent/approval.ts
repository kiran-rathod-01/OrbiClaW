...
//collect all the pending staged action and make as agrouped and organised also for before changes
...
export async function runApprovalFlow(tracker:ActionTracker):Promise<boolean>{
    ...
    if(pending.length === 0){
        console.log(
            chalk.dim("\nNo stage file, folder ,or shell chnages to review.\n")
        );
        return false; 
    }

    const choice = await select({//we give use to chose the change optione if they want how should changes happens
        message:"Apply staged chnages?",
        options:[
    ...
    if(isCancel(choice) || choice === "cancle"){
