...
export class ToolExecutor {//its a Bridge between youer AI tool and the real file system 

    private overlay = new Map<string, string>();//overlay do that if Agent do any think 
    // so overlay stor that state into hash map not go directly store in disk

    private deleted = new Set<string>();//if any state want to delete

    private readonly norm = (rel: string) =>//this is for when we are in windos the windos have 
    // in terminal (\)forword slash but in mac this is oppsite so we are chinging using this
        path.posix.normalize(rel.split(path.sep).join("/")).replace(/^\.\//,"");


    constructor (        
        private readonly tracker :ActionTracker,
        private readonly config : AgentConfig ,//givs root path also , which files should Agent accesses or not
    ){}

    private resolveSafe(rel: string): string {//thi sis helps in codebase if path are not 
    // in woright form so this is return absolute path after clearing thinks
    ...
    private excluded(relPath: string): boolean{//its ignoring which files to accesses or not
        const norm = this.norm(relPath);//normalizing the path using norm 
    ...
    private assertNotExcluded(rel: string, op: string): void {//its check thgis file excluded or not rel-> is path of file
        //and op-> is oprations
    ...
    getEffectiveText(rel: string): string | undefined {
        //its going to inMeory stage and check the  overlay and deletated stages in given path the data exist or not
        // its shout be priorty givenn by new Data that AI Agent was changed
    ...
    readfile(rel: string):string {//read files by given path
        ...
        this.tracker.log({//each data should maintain in tracker by log's to maintain each state
            type: "code_analysis", 
            path: this.norm(rel),
    ...
    createFile(rel: string, content: string): string {//creat Fils by given path
    ...
    modifyFile(rel: string, content: string): string{ // modifie Files y given path
    ...
    deleteFile(rel: string): string {// delete files by given path
    ...
    createFolder (rel: string): string { // creay Folders by given path 
    ...
    listFiles(rel: string, recursive: boolean): string {//this is list the all files what i have
        ...
        this.tracker.log({//and track all the list files
            type: "code_analysis", 
            path: this.norm(rel),
    ...
    searchFiles( //search by a sfaciic word i current file
        rootRel: string,//root path
        globPattern: string, // give some pattern so Agent can search on given pattern 
        contentQuery?: string,
        ): string {
            ...
            return out || "(no matches)"; // if i will not get so i will return no match
        }
    ...
    skillRoots(): string[] {//skills should be currenmt working directry if not agent should be ignor 
        const extra =
    ...
    listSkills(): string {
        ...
        for (const root of this.skillRoots()) {//using loop check all skills get and replay
    ...
    readSkill(skillPath: string): string {//we give some skill pathat to read then AI do there job internully 
        ...
        if (!allowed)
            throw new Error("read_skill: outside skill roots");
    ...
    applyApprovedFromTracker(): { errors: string[] } {//helps to creat files 
        const errors: string[] = [];
        const all = [...this.tracker.getAction()];//get those exluded files in action-tracker 
        ...
        for (const a of fileOps) {
            lastByPath.set(this.norm(a.path), a);
        }
